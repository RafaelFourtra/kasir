...
class UserOrder extends Controller
{
    ...
    public function riwayat_order(Request $req){
        ...
        foreach($req->checkbox as $key=>$name){
            $no_po = riwayat_order::whereDate('tanggal', Carbon::today())->count();
        $no_po++;
        $no_nota_po = date('ymd') . str_pad($no_po, 3, 0, STR_PAD_LEFT);
            $insert = [
                'menu' => $req->checkbox[$key],
                'jumlah_pesanan' => $req->jumlah_pesanan[$key],
                'harga' => $req->harga[$key],
                'total_harga' => $req->total_harga[$key],
                'no_nota' => $no_nota_po[$key];
            ];

            DB::table('riwayat_order')->insert($insert);
        }
    }
