...
class UserOrder extends Controller
{
    ...
    public function riwayat_order(Request $req){
        ...
        $detail_trans->harga = $req->harga[$id_menu];
        $detail_trans->kwantitas = $req->pesanan[$id_menu];
        $detail_trans->total_harga = $req->total[$id_menu];
    ...
    public function riwayat_tampil(){
        ...
        return view("userOrder.riwayat_user")->with("transaksi",$lol);
    }

    public function riwayat_tampil2(){
        $lol = riwayat_order::all();
        return view("userOrder.server")->with("transaksi",$lol);
    }

    public function detail_tampil($id){
        $transaksi = riwayat_order::with("detail_riwayat")->get();
        $detailorder = detail_riwayat::where('id_transaksi_order', $id)->get();
        return view("userOrder.riwayat_user", ["transaksi"=>$transaksi,"detailorder"=>$detailorder]);
    }

    public function hapus_data($id){
        $table = riwayat_order::find($id);
        $table->delete();
        return redirect()->back();
    }
}
