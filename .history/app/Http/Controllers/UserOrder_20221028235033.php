...
class UserOrder extends Controller
{
    ...
    public function riwayat_order(Request $req){
        ...
        if(is_array)
       foreach($menu_data as $key => $menus){
        $tambahData['menu'] = $menus;
        $tambahData['jumlah_pesanan'] = $req->jumlah_pesanan[$key];
        $tambahData['harga'] = $req->harga[$key];
        $tambahData['total_harga'] = $req->total_harga[$key];
        $tambahData['no_nota'] = $no_nota_po[$key];

        riwayat_order::create($tambahData);
       }
       DB::commit();
        return redirect()->back();
    }
