...
class UserOrder extends Controller
{
    ...
    public function riwayat_order(Request $req){
        ...
        for($i = 0; $i < count($menu); $i++){
            ...
            $riwayat = riwayat_order::find($id_menu);
            $riwayat->menu = $menu[$i][$id_menu];
            $riwayat->jumlah_pesanan = $pesanan[$i][$id_menu];
            $riwayat->harga = $harga[$i][$id_menu];
            $riwayat->total_harga = $total[$i][$id_menu];
