...
<style>
    ...
    input.uang{
        ...
        <div id="tambah" class="tambah_riwayat">
            ...
            <form action="/tampil/{id}" method="POST" align="center" name="frm">
                @method('PUT')
                @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <table class="table table-striped" style="width: 28rem; margin-left: 25rem;width: 300rem;">
                                    ...
                                    <tbody id="{{$no3++}}" style="display: none; margin-left: 5rem;">
                                        ...
                                        <td><input class="checkbox" type="checkbox" id="menu" name="menu[]" value="{{$key->menu}}" checked>{{$key->menu}}</td>
                                        <td><input name="pesanan[]" type="number" id="{{$no12++}}" style="width: 50px;text-align: center;" value="1"></td>
                                        <td><input class="checkbox" type="checkbox" name="harga[]" class="harga" value="{{$key->harga}}" checked>Rp.{{number_format($key->harga)}}</td>
                                        <td><b>Rp.</b><input class="checkbox2" type="text" name="total[]" id="{{$no15++}}" value="{{$key->harga}}" checked><b name="total" id="{{$no10++}}">{{number_format($key->harga)}}</b></td>
                                        
                                     </tr>
                                    </tbody>
                                    @endforeach
                                    <input class="checked-sum" type="text">
                                </tbody>
                            </thead>
                        </table>
                        </div>
...
function checkMe(id,pid){
    ...
    $(document).ready(function(){
        $("#{{$no13}},#{{$no9}},#{{$no22}},#{{$no23}}").keyup(function(){
            var total=0;
            var total_jumlah = 0;
            var x = Number($("#{{$no13++}}").val());
            var y = Number($("#{{$no9++}}").val());

            var o = Number($("#{{$no22++}}").val());
            var h = Number($("#{{$no23++}}").val());

            var total=x *  y;
            var total_jumlah = total_jumlah += total;

            $('#{{$no11++}}').html(total);
            $('#{{$no16++}}').val(total);
            $(".checked-sum").val(total_jumlah);
            $('.checked-sum').each(function(i, e){
